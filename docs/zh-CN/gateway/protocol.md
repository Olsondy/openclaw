...
- 节点可以调用 `skills.bins` 来获取当前的 skill 可执行文件列表，
  用于自动允许检查。
- 节点可以调用 `channel.auth.status` 查询当前是否存在待完成的渠道授权
  （`required=true`）。

### 节点专用渠道授权通知

- Gateway 网关会以 `type:"event"` 帧向当前已连接的 `role: node` 客户端
  发送 `channel.auth.required` 与 `channel.auth.resolved`。
- 这些事件仅用于通知，故意不携带 payload；节点在重连后应主动调用
  `channel.auth.status` 恢复状态。
